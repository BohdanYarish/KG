...
const AffineTransformationsComponent: React.FC<AffineProps> = (
  ...
  useEffect(() => {
    setHexagonPoints([
      [
        translatePoint(hexagonPoint).x,
        translatePoint(hexagonPoint).y + radius,
        1,
      ],
      [
        translatePoint(hexagonPoint).x + (radius * Math.sqrt(3)) / 2,
        translatePoint(hexagonPoint).y + radius / 2,
        1,
      ],
      [
        translatePoint(hexagonPoint).x + (radius * Math.sqrt(3)) / 2,
  ...
  const translatePoint = (point: Point, gridSnapSize = snapSize): Point => {
    return new Point(
      props.width / 2 + point.x * gridSnapSize,
      props.height / 2 - point.y * gridSnapSize
    );
  };

  const [hexagonPoints, setHexagonPoints] = useState([
    [
      translatePoint(hexagonPoint).x,
      translatePoint(hexagonPoint).y + radius,
      1,
  ...
  const [hexagonCenter, setHexagonCenter] = useState(
    translatePoint(hexagonPoint)
  );
  ...
  const [movebackMatrix, setMoveBackMatrix] = useState([
    [1, 0, 0],
    [0, 1, 0],
    [translatePoint(hexagonPoint).x, translatePoint(hexagonPoint).y, 1],
  ]);
  ...
  const [rotateMatrix, setRotateMatrix] = useState([
    [
      Math.cos(degreesToRadians(rotationAngle)),
      Math.sin(degreesToRadians(rotationAngle)),
      0,
    ],
    [
      -Math.sin(degreesToRadians(rotationAngle)),
    ...
    for (let i = 0; i < props.width / 2 / snapSize; i++) {
      lines.push(
        new Konva.Line({
          id: `y-neg-${i}`,
          points: [
            props.width / 2 - Math.round(i * snapSize),
            0,
            props.width / 2 - Math.round(i * snapSize),
            props.height,
          ],
    ...
    for (let j = 0; j < props.height / 2 / snapSize; j++) {
      lines.push(
        new Konva.Line({
          ...
          points: [
            0,
            props.height / 2 - Math.round(j * snapSize),
            props.width,
            props.height / 2 - Math.round(j * snapSize),
          ],
          stroke: "hsl(0, 0%, 80%)",
  ...
  const handleClick = () => {
    ...
    spin = setInterval(() => {
      ...
      setHexagonCenter(
        new Point(
          (hexagonPoints[0][0] + hexagonPoints[3][0]) / 2,
        ...
        <div className="d-flex flex-column">
          <div className="mt-2 text-left">
            <h3>
              ...
              <i
                ...
                data-tip="Set the center coordinates of the hexagon"
              />
            </h3>
          ...
          <div className="mt-2 text-left">
            <h3>
              ...
              <i
                className="fas fa-question-circle fa-xs"
                data-tip="Set the hexagon radius"
              />
            </h3>
            <ReactTooltip place="right" effect="solid" />
          </div>
          ...
          <div className="mt-2 text-left">
            <h3>
              ...
              <i
                className="fas fa-question-circle fa-xs"
                data-tip="Choose the point you want the hexagon to rotate around"
              />
            </h3>
            <ReactTooltip place="right" effect="solid" />
          </div>
          ...
          <div className="mt-2 text-left">
            <h3>
              ...
              <i
                className="fas fa-question-circle fa-xs"
                data-tip="Set the rotation angle magnitude of the hexagon transformation"
              />
            </h3>
            <ReactTooltip place="right" effect="solid" />
          </div>
          ...
          <div className="mt-2 text-left">
            <h3>
              ...
              <i
                className="fas fa-question-circle fa-xs"
                data-tip="Set scale coefficient of the hexagon transformation"
              />
            </h3>
            <ReactTooltip place="right" effect="solid" />
          </div>
          ...
          <div className="mt-2 text-left">
            <MDBBtn color="elegant" block onClick={handleClick}>
              Transform
            </MDBBtn>
          </div>
        </div>
